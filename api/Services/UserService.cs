...
namespace api.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task RestoreProfileAsync(ClaimsPrincipal user)
        {
            _logger.LogInformation("Khôi phục hồ sơ người dùng");
            var account = await _authService.GetCurrentUserAsync(user);
            ...
            var accountEntity = await _unitOfWork.Repository<Account>()
                .FindSingleAsync(a => a.AccountId == account.AccountId && a.DeletedAt != null);
            if (accountEntity == null)
            {
                _logger.LogWarning("Tài khoản không tìm thấy hoặc chưa bị xóa: AccountId {AccountId}", account.AccountId);
                throw new ArgumentException("Account not found or not deleted.");
            }
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    if (account.Role == "User")
                    {
                        ...
                        {
                            _logger.LogWarning("Không tìm thấy thông tin người dùng cho AccountId: {AccountId}", account.AccountId);
                            throw new ArgumentException("User information not found or not deleted.");
                        }
                    ...
                    else if (account.Role == "Owner")
                    {
                        ...
                        {
                            _logger.LogWarning("Không tìm thấy thông tin chủ sân cho AccountId: {AccountId}", account.AccountId);
                            throw new ArgumentException("Owner information not found or not deleted.");
                        }
                    ...
                    accountEntity.DeletedAt = null;
                    _unitOfWork.Repository<Account>().Update(accountEntity);
                    await _unitOfWork.SaveChangesAsync();
                    await _unitOfWork.CommitTransactionAsync();
                    _logger.LogInformation("Khôi phục hồ sơ thành công cho {Email}", account.Email);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Lỗi khi khôi phục hồ sơ cho {Email}. Rollback transaction.", account.Email);
